...
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_bin COMMENT='여행기 정보';

-- comment
create table `comment`(
	`comment_id` int not null auto_increment comment '댓글 아이디',
	`attraction_id` int not null comment '여행지 아이디',
	`user_email` varchar(50) not null comment '댓글 등록자',
...
create table `user`(
	...
	`password` varchar(50) not null comment '유저 비밀번호',
